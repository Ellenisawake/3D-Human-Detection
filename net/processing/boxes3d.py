...
def top_box_to_box3d(boxes):
    ...
    for n in range(num):
        ...
        for k in range(4):
            ...
            boxes3d[n,k,  :] = x,y,-0.05  ## <todo>
            boxes3d[n,4+k,:] = x,y,0.18
...
def draw_rgb_projections(image, projections, color=(255,255,255), thickness=2, darker=0.7):
    ...
    num=len(projections)
    for n in range(num):
        qs = projections[n].astype(np.int32)
        for k in range(0,4):
            #http://docs.enthought.com/mayavi/mayavi/auto/mlab_helper_functions.html
            i,j=k,(k+1)%4
            cv2.line(img, (qs[i,0],qs[i,1]), (qs[j,0],qs[j,1]), color, thickness, lineType=cv2.LINE_AA)
